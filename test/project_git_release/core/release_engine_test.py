...
from project_git_release.classes import GitHashAndMsg, CommitDetails, ShortCommitData
...
@pytest.fixture
def release_engine(connector, git_commander, temp_dir, release_config, changelog_generator, gen_version_file):
    return ReleaseEngine(connector=connector, git_commander=git_commander, temp_dir=temp_dir, config=release_config)


def test_no_latest_release_and_no_new_commit(release_engine, connector, log_info_spy):
    connector.get_latest_release = MagicMock(return_value=None)

    with pytest.raises(SystemExit) as se:
        release_engine.update_version()
        assert 0 == se.value.code

    log_info_spy.assert_any_call("There is no commit since the latest release. Quitting...")


def test_no_latest_release_and_no_new_commit_error(release_engine, connector, git_commander, log_error_spy):
    connector.get_latest_release = MagicMock(return_value=None)
    git_commander.get_commits_since_latest_release.return_value = None
...
def test_no_latest_release_and_has_no_valid_commits(release_engine, git_commander, connector, log_info_spy):
    connector.get_latest_release = MagicMock(return_value=None)
    git_commander.get_file_history.return_value = [
...
def test_first_release_no_commit_after(release_engine, git_commander, connector, log_info_spy):
    connector.get_latest_release = MagicMock(return_value=None)
    git_commander.get_file_history.return_value = [
...
def test_first_release_has_commit_after(mocker, release_engine, git_commander, connector, log_info_spy):
    mocker.patch.object(connector, "get_latest_release", return_value=MagicMock(return_value=None))
    git_commander.get_file_history.return_value = [
        ShortCommitData(message="chore: Release version 0.1.0", sha="sha_02")
    ]
    git_commander.get_commits_since_latest_release.return_value = [
    ...
    mocker.patch.object(connector, "get_commit_details", side_effect=commit_details_se)
