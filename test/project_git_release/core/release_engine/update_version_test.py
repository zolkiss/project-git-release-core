...
class ReleaseEngineMocks:
    def __init__(self, engine: ReleaseEngine, git_commander: GitCommander, temp_dir: TemporaryDirectory,
                 changelog_generator: ChangelogGenerator, connector: Connector):
        self.engine = engine
...
@pytest.fixture
def engine_mock(connector, git_commander, temp_dir, release_config, changelog_generator,
                gen_version_file) -> ReleaseEngineMocks:
    return ReleaseEngineMocks(
        engine=ReleaseEngine(connector=connector, git_commander=git_commander, temp_dir=temp_dir,
...
def test_first_release_no_commit_after(engine_mock, log_info_spy):
    engine_mock.connector.get_latest_release = MagicMock(return_value=None)
    engine_mock.git_commander.get_file_history.return_value = [
...
def test_first_release_has_commit_after(mocker, engine_mock, log_info_spy):
